...
{
    Functor&                           F;
    xsigma_thread_local<unsigned char> Initialized;
...
public:
   ...
   */
    ...
    {
        typename xsigma::detail::smp::tools_Lookup_For<Functor>::type fi(f);
    ...
    {
        typename xsigma::detail::smp::tools_Lookup_For<Functor const>::type fi(f);
        fi.For(first, last, grain);
    }
    ///@}
   ...
   */
    ...
    static xsigma::detail::smp::resolvedNotInt<Iter> For(
        Iter begin, Iter end, int grain, Functor& f)
    {
        int size = (int)std::distance(begin, end);
        typename xsigma::detail::smp::tools_Lookup_RangeFor<Iter, Functor>::type fi(begin, f);
        ...
        Iter begin, Iter end, int grain, Functor const& f)
    {
        ...
        typename xsigma::detail::smp::tools_Lookup_RangeFor<Iter, Functor const>::type fi(begin, f);
        tools::For(0, size, grain, fi);
    }
    ///@}
   ...
   */
    template <typename Iter, typename Functor>
    static xsigma::detail::smp::resolvedNotInt<Iter> For(Iter begin, Iter end, Functor& f)
    {
        tools::For(begin, end, 0, f);
    }

    template <typename Iter, typename Functor>
    static xsigma::detail::smp::resolvedNotInt<Iter> For(Iter begin, Iter end, Functor const& f)
    {
        tools::For(begin, end, 0, f);
    }
    ///@}
   ...
   * The backend selected with SetBackend() have the priority over XSIGMA_SMP_BACKEND_IN_USE.
   *
   ...
   * to the maximum number of threads or use the XSIGMA_SMP_MAX_THREADS
   * env variable if it is defined.
   *
   * Note: If XSIGMA_SMP_MAX_THREADS env variable is defined the SMPTools will try
   ...
   */
    struct Config
    {
        ...
        std::string Backend            = xsigma::detail::smp::tools_api::GetInstance().GetBackend();
...
#ifndef DOXYGEN_SHOULD_SKIP_THIS
        Config(xsigma::detail::smp::tools_api& API)
   ...
   *
   * Usage example with xsigmaDataArray:
   * \code
   * const auto range0 = xsigma::DataArrayValueRange<1>(array0);
   * auto range1 = xsigma::DataArrayValueRange<1>(array1);
   * tools::Transform(
   *   range0.cbegin(), range0.cend(), range1.begin(), [](double x) { return x - 1; });
   * \endcode
   *
   * Please visit xsigmaDataArrayRange.h documentation for more information and optimisation.
   */
   ...
   * auto range = xsigma::DataArrayTupleRange<1>(array);
