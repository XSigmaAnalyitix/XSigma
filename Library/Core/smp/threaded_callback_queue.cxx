...
{
//=============================================================================
class threaded_callback_queue::ThreadWorker
{
public:
    ThreadWorker(
        xsigma::threaded_callback_queue* queue, std::shared_ptr<std::atomic_int>& threadIndex)
        : Queue(queue), ThreadIndex(threadIndex)
    {
    }

    ThreadWorker(ThreadWorker&& other) noexcept
        : Queue(other.Queue), ThreadIndex(std::move(other.ThreadIndex))
    {
    }
...
private:
   ...
   */
    bool Pop()
    {
        ...
        xsigma::threaded_callback_queue::SharedFutureBasePointer invoker =
            std::move(invokerQueue.front());
   ...
   */
    ...
    xsigma::threaded_callback_queue* Queue;
...
//-----------------------------------------------------------------------------
void threaded_callback_queue::SetNumberOfThreads(int numberOfThreads)
{
    this->PushControl(
        [this, numberOfThreads]()
        {
            ...
            if (size < numberOfThreads)
            {
                ...
                std::generate_n(
                    std::back_inserter(this->Threads),
                    numberOfThreads - size,
                    [this]
                    {
                        auto threadIndex = std::make_shared<std::atomic_int>(
...
//-----------------------------------------------------------------------------
void threaded_callback_queue::Sync(int startId)
{
    std::for_each(
        this->Threads.begin() + startId,
        this->Threads.end(),
        [](std::thread& thread) { thread.join(); });
}

//-----------------------------------------------------------------------------
void threaded_callback_queue::PopFrontNullptr()
{
...
//-----------------------------------------------------------------------------
void threaded_callback_queue::Invoke(ptr_mutable<xsigmaSharedFutureBase> invoker)
{
    (*invoker)();
...
//-----------------------------------------------------------------------------
void threaded_callback_queue::SignalDependentSharedFutures(
    ptr_mutable<xsigmaSharedFutureBase> invoker)
{
    ...
    std::vector<SharedFutureBasePointer> invokersToLaunch;
    {
        ...
        for (auto& dependent : invoker->Dependents)
        {
            ...
            if (dependent->Status.load(std::memory_order_acquire) == ON_HOLD &&
                (dependent->NumberOfPriorSharedFuturesRemaining == 0))
            {
                // Invoker is high priority if it comes from threaded_callback_queue::Wait for example.
    ...
    if (!invokersToLaunch.empty())
    {
        ...
        // 0.
        auto index = this->InvokerQueue.empty() ? static_cast<int>(invokersToLaunch.size())
                                                : this->InvokerQueue.front()->InvokerIndex;
        for (SharedFutureBasePointer& inv : invokersToLaunch)
        {
            assert(
                inv->Status.load(std::memory_order_acquire) == ON_HOLD &&
...
//-----------------------------------------------------------------------------
bool threaded_callback_queue::TryInvoke(ptr_mutable<xsigmaSharedFutureBase> invoker)
{
    if (![this, &invoker]
        {
            ...
            auto index = invoker->InvokerIndex - this->InvokerQueue.front()->InvokerIndex;
