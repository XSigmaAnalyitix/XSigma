...
class XSIGMA_VISIBILITY Section {
    char*        data = nullptr;
    size_t       size = 0;
...
class XSIGMA_VISIBILITY MemFile {
    explicit MemFile(const char* filename_) : fd_(open(filename_, O_RDONLY)), name_(filename_)
    {
        UNWIND_CHECK(fd_ != -1, "failed to open {}: {}", filename_, xsigma::utils::str_error(errno));
        struct stat s{};
        if (-1 == fstat(fd_, &s))
        {
            close(fd_);  // destructors don't run during exceptions
            UNWIND_CHECK(false, "failed to stat {}: {}", filename_, xsigma::utils::str_error(errno));
        }
        n_bytes_ = s.st_size;
        ...
        {
            close(fd_);
            UNWIND_CHECK(false, "failed to mmap {}: {}", filename_, xsigma::utils::str_error(errno));
        }
