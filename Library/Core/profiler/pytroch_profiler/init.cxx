...
#include <xsigma/core/impl/PyInterpreter.h>
#include "util/exception.h" 
#include "util/overloaded.h"
#include <xsigma/csrc/DynamicTypes.h>
#include <xsigma/csrc/autograd/utils/wrap_outputs.h>
#include <xsigma/csrc/jit/python/pybind_utils.h>
#include "collection.h"
#include "python/combined_traceback.h"
#include "python/init.h"
#include "standalone/execution_trace_observer.h"
#include <xsigma/csrc/utils/pybind.h>

class XSIGMA_VISIBILITY THPCapturedTraceback {
    PyObject_HEAD std::shared_ptr<xsigma::CapturedTraceback> data;
};

static int THPCapturedTraceback_traverse(PyObject* self, visitproc visit, void* arg)
{
...
static void THPCapturedTraceback_dealloc(PyObject* self_)
{
    ...
    self->data.~shared_ptr<xsigma::CapturedTraceback>();
    // promptly trigger delayed frees since we have GIL
    xsigma::freeDeadCapturedTracebackFrames();
    PyObject_GC_Del(self);
}

static PyTypeObject THPCapturedTracebackType = {
    PyVarObject_HEAD_INIT(nullptr, 0) "xsigma._C._profiler.CapturedTraceback", /* tp_name */
...
template <>
struct type_caster<std::shared_ptr<xsigma::CapturedTraceback>>
{
public:
    PYBIND11_TYPE_CASTER(
        std::shared_ptr<xsigma::CapturedTraceback>, _("xsigma._C._profiler.CapturedTraceback"));
    ...
    static handle cast(
        std::shared_ptr<xsigma::CapturedTraceback> src,
        return_value_policy /* policy */,
        handle /* parent */)
    {
        auto* r = PyObject_GC_New(THPCapturedTraceback, &THPCapturedTracebackType);
        new (&r->data) std::shared_ptr<xsigma::CapturedTraceback>(std::move(src));
 ...
 * The xsigma.profiler.record_function context manager is slow (~14us on
 ...
 *      rf = xsigma._C._record_function_fast_enter(name)
 *      try:
 *        yield
 *      finally:
 *        xsigma._C._record_function_fast_exit(rf)
 * The C++ implementation here is faster by ~0.2-0.4us per context manager.
 */

namespace
{
class XSIGMA_VISIBILITY RecordFunctionFast {
...
PyObject* RecordFunctionFast_new(PyTypeObject* subtype, PyObject* args, PyObject* kwargs)
{
...
int RecordFunctionFast_init(PyObject* selfGeneric, PyObject* args, PyObject* kwargs)
{
    auto self = (RecordFunctionFast*)selfGeneric;
    // NOLINTNEXTLINE(*-c-arrays*)
    constexpr const char* kwlist[]       = {"name", "input_values", "keyword_values", nullptr};
    ...
    {
        XSIGMA_CHECK(
            THPUtils_checkString(name), "The name passed to RecordFunctionFast must be a string");
        Py_INCREF(name);
        self->name = name;
    }
    if (input_values)
    ...
    if (keyword_values)
    {
        XSIGMA_CHECK(PyDict_Check(keyword_values), "keyword_values must be dict");
...
{
    HANDLE_TH_ERRORS
    if (xsigma::profiler::impl::ProfilerStateBase::get() != nullptr)
    {
        auto self = (RecordFunctionFast*)selfGeneric;
        XSIGMA_CHECK(
            !self->guard,
            "Trying to enter a new record_function_fast context but the guard is unexpectedly "
            "already set");
        auto                                        scope = at::RecordScope::FUNCTION;
        std::vector<at::IValue>                     args;
        std::unordered_map<std::string, at::IValue> kwargs;
        bool                                        profiler_need_input =
            xsigma::autograd::profiler::profilerEnabled() &&
            xsigma::autograd::profiler::getProfilerConfig().report_input_shapes;
        ...
        if (self->input_values != nullptr && profiler_need_input)
        {
            ...
            for (int i = 0; i < PySequence_Fast_GET_SIZE(input_fast.get()); i++)
            {
                ...
                auto      match = xsigma::jit::tryToInferType(item);
                if (match.success())
                {
                    args.push_back(xsigma::jit::toIValue(item, match.type()));
                }
            }
        }
        ...
        if (self->keyword_values != nullptr && profiler_need_input)
        {
            ...
            while (PyDict_Next(self->keyword_values, &pos, &key, &value))
            {
                ...
                else
                {
                    ...
                    auto match = xsigma::jit::tryToInferPrimitiveType(value);
                    if (match.success())
                    {
                        ivalue = xsigma::jit::toIValue(value, match.type());
                    }
                    else if (PyList_Check(value))
                    {
                        ...
                        if (all_strings)
                        {
                            xsigma::List<std::string> string_ivalue_list(string_list);
                            ivalue = at::IValue(string_ivalue_list);
                        }
                        else
                        {
                            XSIGMA_WARN(
                                "Unable to infer type of value in the List for keyword: ", key_str);
                        }
                    }
                    else
                    {
                        XSIGMA_WARN("Unable to infer type of value for keyword: ", key_str);
...
void initPythonBindings(PyObject* module)
{
    ...
    py::class_<ExperimentalConfig>(m, "_ExperimentalConfig")
      ...
      .def(py::pickle(
          ...
          [](const py::tuple& t) { // __setstate__
            XSIGMA_CHECK(t.size() < 5, "Expected at least 5 values in state");
    ...
    py::class_<TensorMetadata>(m, "_TensorMetadata")
        ...
        .def_property_readonly(
            ...
            {
                PyObject* layout_obj = xsigma::autograd::utils::wrap(metadata.layout_);
        ...
        .def_property_readonly(
            ...
            [](const TensorMetadata& metadata)
            {
                return py::reinterpret_borrow<py::object>(
                    xsigma::autograd::utils::wrap(metadata.dtype_));
            })
        ...
        .def_property_readonly(
            ...
            [](const torch_op_t& op)
            {
                ...
                for (const auto& input : op.inputs_)
                {
                    std::visit(
                        xsigma::overloaded(
                            [&](const xsigma::IValue& v) { out.append(xsigma::jit::toPyObject(v)); },
    ...
    py::class_<allocation_t>(m, "_ExtraFields_Allocation")
        .def_property_readonly(
            "ptr", [](const allocation_t& a) { return reinterpret_cast<intptr_t>(a.ptr_); })
    ...
    py::class_<PyFrameState>(m, "_PyFrameState")
        ...
        .def_property_readonly("file_name", [](const PyFrameState& s) { return s.filename_.str(); })
        .def_property_readonly(
            "function_name", [](const PyFrameState& s) { return s.funcname_.str(); });
    ...
    py::class_<NNModuleInfo>(m, "_NNModuleInfo")
        .def_property_readonly(
            ...
            [](const NNModuleInfo& s)
            {
                ...
                {
                    out.append(py::make_tuple(p.name_, p.metadata_, p.grad_metadata_));
                }
                return out;
            })
        .def_property_readonly("cls_name", [](const NNModuleInfo& s) { return s.cls_name_.str(); })
    ...
    py::class_<Result, std::shared_ptr<Result>>(m, "_ProfilerEvent")
        ...
        .def_property_readonly(
            ...
            {
                return py::make_tuple(
                    r.tag(), py::cast(r.extra_fields_, py::return_value_policy::reference));
            })
        .def_property_readonly(
            "id",
            [](const Result& r) { return reinterpret_cast<intptr_t>(r.shared_from_this().get()); })
        .def_property_readonly("parent", [](const Result& r) { return r.parent_.lock(); })
        ...
        .def_property_readonly(
            "duration_time_ns", [](const Result& r) { return r.endTimeNS() - r.start_time_ns_; });

    // PyTorch profiler execution trace internal interface.
    m.def(
        "_add_execution_trace_observer",
        &xsigma::profiler::impl::addExecutionTraceObserver,
        py::arg("output_file_name"));
    m.def("_remove_execution_trace_observer", &xsigma::profiler::impl::removeExecutionTraceObserver);
    m.def("_enable_execution_trace_observer", &xsigma::profiler::impl::enableExecutionTraceObserver);
    m.def(
        "_disable_execution_trace_observer", &xsigma::profiler::impl::disableExecutionTraceObserver);
    m.def(
        "_set_record_concrete_inputs_enabled_val",
        &xsigma::profiler::impl::set_record_concrete_inputs_enabled_val);
    m.def("_set_fwd_bwd_enabled_val", &xsigma::profiler::impl::set_fwd_bwd_enabled_val);
    m.def("_set_cuda_sync_enabled_val", &xsigma::profiler::impl::set_cuda_sync_enabled_val);

    XSIGMA_CHECK(PyType_Ready(&THPCapturedTracebackType) >= 0);
    PyModule_AddObject(m.ptr(), "CapturedTraceback", (PyObject*)&THPCapturedTracebackType);
    m.def(
        "gather_traceback",
    ...
    m.def(
        ...
        [](const std::vector<uint64_t>& frames, const std::string& mode_s)
        {
            ...
            xsigma::unwind::Mode mode = xsigma::unwind::Mode::addr2line;
            if (mode_s == "fast")
            {
                mode = xsigma::unwind::Mode::fast;
            }
            else if (mode_s == "addr2line")
            {
                mode = xsigma::unwind::Mode::addr2line;
            }
            else if (mode_s == "dladdr")
            {
                mode = xsigma::unwind::Mode::dladdr;
            }
            else
            {
                XSIGMA_CHECK(false, "unexpected mode ", mode_s);
            }
    ...
    static PyTypeObject RecordFunctionFast_Type = {PyVarObject_HEAD_INIT(nullptr, 0)};

    RecordFunctionFast_Type.tp_name      = "xsigma._C._profiler.RecordFunctionFast",
    ...
    if (PyModule_AddObject(m.ptr(), "_RecordFunctionFast", (PyObject*)&RecordFunctionFast_Type) !=
        0)
    {
