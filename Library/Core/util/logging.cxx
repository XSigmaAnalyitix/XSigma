...
constexpr double Frequency = 1e9;
...
class XSIGMALogSinks
{
public:
    // Gets the XSIGMALogSinks instance. This is the entry point for using this class.
    static XSIGMALogSinks& Instance();

    // Adds a log sink. The sink argument must not be a nullptr. XSIGMALogSinks
    ...
    // until XSIGMALogSinks::Remove is called for the same pointer value.
    void Add(XSIGMALogSink* sink);

    // Removes a log sink. This will also erase the sink object. The pointer
    // to the sink becomes invalid after this call.
    void Remove(XSIGMALogSink* sink);

    // Gets the currently registered log sinks.
    std::vector<XSIGMALogSink*> GetSinks() const;
    ...
    void Send(const XSIGMALogEntry& entry);

private:
    XSIGMALogSinks();
    void SendToSink(XSIGMALogSink& sink, const XSIGMALogEntry& entry);

    std::queue<XSIGMALogEntry> log_entry_queue_;
    static const size_t        kMaxLogEntryQueueSize = 128;

    mutable std::mutex          mutex_;
    std::vector<XSIGMALogSink*> sinks_;
};

XSIGMALogSinks::XSIGMALogSinks()
{
#ifndef NO_DEFAULT_LOGGER
    static auto* default_sink = new XSIGMADefaultLogSink();
    sinks_.emplace_back(default_sink);
#endif
}

XSIGMALogSinks& XSIGMALogSinks::Instance()
{
    static XSIGMALogSinks* instance = new XSIGMALogSinks();
    return *instance;
}

void XSIGMALogSinks::Add(XSIGMALogSink* sink)
{
    ...
    if (sinks_.size() == 1)
    {
        while (!log_entry_queue_.empty())
        {
            ...
            {
                SendToSink(*s, log_entry_queue_.front());
            }
...
void XSIGMALogSinks::Remove(XSIGMALogSink* sink)
{
    assert(sink != nullptr && "The sink must not be a nullptr");
...
std::vector<XSIGMALogSink*> XSIGMALogSinks::GetSinks() const
{
    std::lock_guard<std::mutex> lock(mutex_);
    return sinks_;
}

void XSIGMALogSinks::Send(const XSIGMALogEntry& entry)
{
    std::lock_guard<std::mutex> lock(mutex_);
...
void XSIGMALogSinks::SendToSink(XSIGMALogSink& sink, const XSIGMALogEntry& entry)
{
    sink.Send(entry);
    sink.WaitTillSent();
}
...
// If the environment variable XSIGMA_CPP_VLOG_FILENAME is set, all VLOG
...
{
    vlog_file_name = std::getenv("XSIGMA_CPP_VLOG_FILENAME");
    vlog_file_ptr  = vlog_file_name == nullptr ? nullptr : fopen(vlog_file_name, "w");
...
struct StringData
{
    struct Hasher
    {
        size_t operator()(const StringData& sdata) const
        {
            ...
            for (const char* top = d + sdata.size; d < top; ++d)
            {
                hash = ((hash << 5) + hash) + (*d);
            }
            return hash;
...
// XSIGMA_CPP_VMODULE environment variable; ownership is transferred to the caller.
VmoduleMap* VmodulesMapFromEnv()
{
    // The value of the env var is supposed to be of the form:
    //    "foo=1,bar=2,baz=3"
    const char* env = std::getenv("XSIGMA_CPP_VMODULE");
    if (env == nullptr)
    {
        // If there is no XSIGMA_CPP_VMODULE configuration (most common case), return
        // nullptr so that the ShouldVlogModule() API can fast bail out of it.
        return nullptr;
    }
    // The memory returned by std::getenv() can be invalidated by following std::getenv() or
...
{
    const char* tf_env_var_val = std::getenv("XSIGMA_CPP_LOG_THREAD_ID");
    return tf_env_var_val == nullptr ? false
...
LogSeverityAtLeast MinLogLevelFromEnv()
{
    ...
    return std::LogSeverityAtLeast::kInfinity;
#else
    const char* tf_env_var_val = std::getenv("XSIGMA_CPP_MIN_LOG_LEVEL");
    return static_cast<LogSeverityAtLeast>(LogLevelStrToInt(tf_env_var_val));
#endif
}
...
int MaxVLogLevelFromEnv()
{
    ...
    return 0;
#else
    const char* tf_env_var_val = std::getenv("XSIGMA_CPP_MAX_VLOG_LEVEL");
    return LogLevelStrToInt(tf_env_var_val);
#endif
}

LogMessage::LogMessage(const char* fname, int line, LogSeverity severity)
...
LogMessage::~LogMessage()
{
    ...
    static LogSeverityAtLeast min_log_level = MinLogLevelFromEnv();
    if ((int)severity_ >= (int)min_log_level)
    {
        GenerateLogMessage();
    }
}
...
{
    XSIGMALogSinks::Instance().Send(XSIGMALogEntry(severity_, fname_, line_, str()));
}
...
bool LogMessage::VmoduleActivated(const char* fname, int level)
{
    ...
    if XSIGMA_LIKELY (vmodules == nullptr)
    {
        return false;
    }
...
LogMessageFatal::LogMessageFatal(const char* file, int line)
    : LogMessage(file, line, LogSeverity::kFatal)
{
}
...
void LogString(const char* fname, int line, LogSeverity severity, const std::string& message)
{
...
bool LogEveryPow2State::ShouldLog(XSIGMA_UNUSED int ignored)
{
...
bool LogEveryNSecState::ShouldLog(double seconds)
{
    ...
    const int64_t now_cycles  = std::chrono::steady_clock::now().time_since_epoch().count();
...
{
    internal::XSIGMALogSinks::Instance().Add(sink);
}

void XSIGMARemoveLogSink(XSIGMALogSink* sink)
{
    internal::XSIGMALogSinks::Instance().Remove(sink);
}

std::vector<XSIGMALogSink*> XSIGMAGetLogSinks()
{
    return internal::XSIGMALogSinks::Instance().GetSinks();
}

void XSIGMADefaultLogSink::Send(const XSIGMALogEntry& entry)
{
    ...
    case LogSeverity::kFatal:
        ...
        break;
    default:
        if (entry.log_severity() < LogSeverity::kInfo)
        {
    ...
    {
        std::stringstream ss;
        ss << " " << std::setw(7) << std::this_thread::get_id();
        snprintf(tid_buffer, sizeof(tid_buffer), "%s", ss.str().c_str());
    }
...
void UpdateLogVerbosityIfDefined(XSIGMA_UNUSED const char* env_var) {}
