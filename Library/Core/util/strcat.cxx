...
namespace xsigma::strings
{

AlphaNum::AlphaNum(Hex hex)
{
    char* const end    = &digits_[xsigma::numbers::kFastToBufferSize];
    char*       writer = end;
...
static char* Append4(
    char* out, const AlphaNum& x1, const AlphaNum& x2, const AlphaNum& x3, const AlphaNum& x4)
{
    if (x1.data() != nullptr)
    {
        memcpy(out, x1.data(), x1.size());
...
{
    return std::string(a.data(), a.size());  //NOLINT
}
...
std::string StrCat(const AlphaNum& a, const AlphaNum& b)
{
    ...
    XSIGMA_CHECK(out == begin + result.size());
    return result;
}
...
std::string StrCat(const AlphaNum& a, const AlphaNum& b, const AlphaNum& c)
{
    ...
    char*       out   = Append2(begin, a, b);
    out               = Append1(out, c);
    XSIGMA_CHECK_EQ(out, begin + result.size());
    return result;
}
...
struct ResizeUninitializedTraits<
    string_type,
    std::void_t<decltype(std::declval<string_type&>().__resize_default_init(237))> >
{
    using HasMember = std::true_type;
    static void Resize(string_type* s, size_t new_size) { s->__resize_default_init(new_size); }
};
...
// increasing the std::string size by a small amount is O(1), in contrast to
// O(str->size()) in the case of precise growth.
// TODO(b/217943845): Would be better to use std::strings so we don't need to
...
// exponential growth so that the amortized complexity of increasing the std::string
...
// Therefore, check for this in verbose mode.  Use unsigned math so we only have
// to do one comparison.
#define DCHECK_NO_OVERLAP(dest, src) \
    XSIGMA_CHECK(uintptr_t((src).data() - (dest).data()) >= uintptr_t((dest).size()))
...
void StrAppend(std::string* result, const AlphaNum& a, const AlphaNum& b, const AlphaNum& c)
{
    ...
    std::string::size_type old_size = result->size();
    STLStringResizeUninitializedAmortized(result, old_size + a.size() + b.size() + c.size());
    char* const begin = &*result->begin();
    char*       out   = Append2(begin + old_size, a, b);
    out               = Append1(out, c);
    XSIGMA_CHECK_EQ(out, begin + result->size());
...
void StrAppend(
    std::string* result, const AlphaNum& a, const AlphaNum& b, const AlphaNum& c, const AlphaNum& d)
{
    DCHECK_NO_OVERLAP(*result, a);
    DCHECK_NO_OVERLAP(*result, b);
    DCHECK_NO_OVERLAP(*result, c);
