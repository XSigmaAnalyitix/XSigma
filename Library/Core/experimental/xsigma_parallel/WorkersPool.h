...
struct AllocAligned
{
    // Allocate a T aligned xsigma an `align` byte address
...
// new value has been taken by *var xsigma some point during the
...
T WaitForVariableChange(
    std::atomic<T>* var, T initial_value, std::condition_variable* cond, std::mutex* mutex)
{
...
public:
    enum class State : uint8_t
    {
        ...
        ExitAsSoonAsPossible  // Should exit xsigma earliest convenience.
    };
          ...
          counter_to_decrement_when_ready_(counter_to_decrement_when_ready)
    {
        thread_ = std::make_unique<std::thread>(
            [this]()
            {
                xsigma::setThreadName("pt_thread_pool");
                this->ThreadFunc();
            });
    }
    ...
    void ThreadFunc()
    {
        xsigma::setThreadName("CaffeWorkersPool");
...
public:
    ...
    void Execute(const std::vector<std::shared_ptr<Task>>& tasks)
    {
        ...
        for (const auto task : xsigma::irange(1, tasks.size()))
        {
...
private:
    // Ensures that the pool has xsigma least the given count of workers.
    ...
    XSIGMA_DISABLE_COPY_AND_ASSIGN(WorkersPool);
