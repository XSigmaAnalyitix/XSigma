    ...
    size_t                           range)
{
    ...
    else
    {
        reinterpret_cast<caffe2::ThreadPool*>(threadpool)
            ->run(
                [function, argument](int threadId, size_t workId) { function(argument, workId); },
                range);
    }
#else
    reinterpret_cast<caffe2::ThreadPool*>(threadpool)
...
legacy_pthreadpool_t legacy_pthreadpool_create(size_t threads_count)
{
    ...
    return reinterpret_cast<legacy_pthreadpool_t>(
        caffe2::ThreadPool::createThreadPool(threads_count));
}
...
void legacy_pthreadpool_destroy(legacy_pthreadpool_t pthreadpool)
{
    if (pthreadpool)
    {
        caffe2::ThreadPool* threadpool = reinterpret_cast<caffe2::ThreadPool*>(pthreadpool);
        delete threadpool;
    }
}
