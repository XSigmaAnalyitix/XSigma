...
void PThreadPool::run(const std::function<void(size_t)>& fn, const size_t range)
{
...
PThreadPool* pthreadpool(size_t thread_count)
{
    ...
    std::call_once(flag, []() { pthread_atfork(nullptr, nullptr, child_atfork); });
#endif
    if (XSIGMA_UNLIKELY(leak_corrupted_threadpool))
    {
