...
public:
    ...
    ExprPtr initializer() const { return init_; }
    ...
    static ExprHandle getReduceBody(
        const std::function<ExprHandle(ParameterList&)>& func, const std::vector<VarHandle>& vars)
    {
        return func(vars);
    }

    static ExprHandle getReduceBody(
        const std::function<ExprHandle(const VarHandle&)>& func, const std::vector<VarHandle>& vars)
    {
        if (vars.size() != 1)
        {
    ...
    static ExprHandle getReduceBody(
        const std::function<ExprHandle(const VarHandle&, const VarHandle&, const VarHandle&)>& func,
        const std::vector<VarHandle>&                                                          vars)
    {
        if (vars.size() != 3)
        {
    ...
    static ExprHandle getReduceBody(
        const std::function<ExprHandle(
            const VarHandle&, const VarHandle&, const VarHandle&, const VarHandle&)>& func,
        const std::vector<VarHandle>&                                                 vars)
    {
        if (vars.size() != 4)
        {
    ...
    {
        ExprHandle accum = ExprHandle(alloc<Load>(body.dtype(), accumulator, output_args));
...
{
public:
    ReduceOp(const ExprPtr& body, std::vector<VarPtr> reduce_args, Reducer reducer)
    ...
    ExprPtr body() const { return body_; }
    ...
    const std::vector<VarPtr>& reduce_args() const { return reduce_args_; }

    void   setAccBuf(BufHandle acc_buf) { acc_buf_ = acc_buf.node(); }
    BufPtr getAccBuf() { return acc_buf_; }

    void   setResultBuf(BufHandle buf) { result_buf_ = buf.node(); }
    BufPtr getResultBuf() { return result_buf_; }

    void    setRiOperand(ExprHandle ri_operand) { ri_operand_ = ri_operand.node(); }
    ExprPtr getRiOperand() { return ri_operand_; }
...
{
public:
    Sum() : Reducer(ExprHandle(0), [](const ExprHandle& a, const ExprHandle& b) { return a + b; })
    {
    }
};
...
{
public:
    StmtPtr expand(const StmtPtr& s) { return s->accept_mutator(this); }

    ExprPtr mutate(const ReduceOpPtr& v) override { return v->body(); }
};
