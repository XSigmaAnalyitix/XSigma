...
template <class ExprType>
Dtype promoteTypesMap(const ExprPtr& s, std::unordered_map<SimplifierHashType, ExprType>& m)
{
    Dtype t     = s->dtype();
    bool  first = true;
    for (auto& e : m)
...
public:
    ...
    ExprPtr                     scalar() const { return scalar_; }
    const std::vector<ExprPtr>& variables() const { return variables_; }
    HashProvider&               hasher() const { return hasher_; }
...
private:
    ...
    void addComponent() {}
    void addComponent(ExprPtr e) { variables_.push_back(std::move(e)); }
    template <class... Es>
...
public:
    ...
    Polynomial(HashProvider& hasher, const ExprPtr& s, std::vector<TermPtr> v)
        : ExprNodeBase(promoteTypesVec(s, v)), variables_(std::move(v)), scalar_(s), hasher_(hasher)
    {
        sort();
    }
    ...
    ExprPtr                     scalar() const { return scalar_; }
    const std::vector<TermPtr>& variables() const { return variables_; }
    HashProvider&               hasher() const { return hasher_; }
...
private:
    ...
    void addTerm(TermPtr t) { variables_.push_back(std::move(t)); }
    template <class... Ts>
...
public:
    ...
    HashProvider& hasher() { return hasher_; }
...
public:
    ...
    bool check_safe() { return eliminated_allocations_.empty(); }
...
public:
    ...
    static ExprPtr    simplify(ExprPtr e);
    static ExprHandle simplify(const ExprHandle& e) { return ExprHandle(simplify(e.node())); }
};
