...
{
    TORCH_CHECK(range.second <= out.size());
    TORCH_CHECK(range.second - range.first == t.size(), "inconsistent range for TensorList output");
    ...
    bool                         keepdim)
{
    ...
    {
        auto sumpow_t =
            (self_p.abs().pow_(p - 1) * at::real(self_p.sgn() * self_t.conj())).sum(dim, keepdim);
    ...
    {
        auto sumpow_t =
            (self_p.abs().pow_(p - 2) * at::real(self_p * self_t.conj())).sum(dim, keepdim);
        auto out = sumpow_t / norm.pow(p - 1);
        return out.masked_fill_(norm == 0, 0);
    }
}
    ...
    const Tensor& grad, const Tensor& self, const Tensor& exponent, const Tensor& result)
{
    ...
    auto out =
        grad *
        at::where(cond, at::scalar_tensor(0.0, grad.options()), (result * self_.log()).conj());
    ...
    const Tensor& grad, const Scalar& base, const Tensor& exponent, const Tensor& result)
{
    ...
    if (base.equal(0.0))
    {
        ...
        auto out =
            grad *
            at::where(cond(exponent), at::zeros({}, grad.options()), grad_lambda(result, base_));
        return handle_r_to_c(exponent, std::move(out));
    }
    else
    {
...
Tensor masked_fill_backward(const Tensor& grad, const Tensor& mask)
{
    ...
    // data-dependent
    return areAnyTensorSubclassLike({grad, mask}) ? at::where(mask, grad, 0).sum()
...
Tensor sum_backward(
    const Tensor& grad, c10::SymIntArrayRef sizes, OptionalIntArrayRef opt_dims, bool keepdim)
{
...
Tensor sum_backward(
    const Tensor& grad, c10::SymIntArrayRef sizes, c10::IntArrayRef dims, bool keepdim)
{
...
Tensor nansum_backward(
    const Tensor& grad, const Tensor& self, at::OptionalIntArrayRef dims, bool keepdim)
{
...
Tensor unbind_backward(const variable_list& grads, int64_t dim)
{
    ...
    auto grads_tensors = fmap(
        grads,
        [&](const Variable& v)
        { return (v.defined() ? static_cast<Tensor>(v) : at::zeros({}, o).expand_symint(sizes)); });
...
Tensor unsqueeze_to(const Tensor& self, IntArrayRef dims, c10::SymIntArrayRef sym_sizes)
{
    ...
    const std::vector<ScalarType>&           dtypes)
{
    ...
    for (const auto i : c10::irange(sizes.size()))
    {
        // R -> C
        Tensor grad_val =
            (!at::isComplexType(dtypes[i]) && grad_is_complex) ? real_view_of_grad : grad;
...
{
    auto bias_t_opt = bias_t.defined() ? std::optional<at::Tensor>(bias_t) : std::nullopt;
    return (
    ...
    const Tensor& grad, const Tensor& mat1, const Tensor& mat2, const Scalar& alpha)
{
    if (grad.layout() == c10::kStrided && mat2.layout() == c10::kStrided && mat1.is_sparse())
    {
        ...
        grad.layout() == c10::kStrided && mat2.layout() == c10::kStrided && mat1.is_sparse_csr())
    {
...
Tensor sparse_mask_backward(const Tensor& grad, const Tensor& mask, const c10::Layout self_layout)
{
    ...
    const Tensor& grad, const Tensor& self, const Scalar& p, int64_t dim, const Scalar& maxnorm)
{
    ...
    auto norm = at::linalg_vector_norm(self, p, reduce_dims, /*keepdim=*/true, /*dtype=*/acc_type);
...
Tensor repeat_backward(Tensor grad, c10::SymIntArrayRef repeats, c10::SymIntArrayRef input_shape)
{
...
{
    TORCH_INTERNAL_ASSERT(
        self.layout() == c10::kJagged, "Nested Strided Tensor doesn't support chunk backward.")
    ...
    const at::TensorOptions&                      options)
{
    ...
    for (int64_t i : c10::irange(static_cast<int64_t>(grads.size())))
    {
        ...
        else
        {
            ...
            Tensor zeros_buffer =
                at::zeros({at::native::get_numel_from_nested_size_tensor(nt_split_size)}, options);
...
static bool _maybe_overlapping_memory(c10::SymIntArrayRef sizes, c10::SymIntArrayRef strides)
{
    if (!sizes.empty())
    {
    ...
    const bool    full_matrices)
{
    ...
    auto dP = m < n ? at::matmul(U.mH(), at::matmul(dA, V)) : at::matmul(at::matmul(U.mH(), dA), V);

    auto dS = is_complex ? at::real(dP.diagonal(0, -2, -1)) : dP.diagonal(0, -2, -1);
    ...
    const Tensor& Vh)
{
    ...
    {
        return m >= n ? at::matmul(U, gS.unsqueeze(-1) * Vh) : at::matmul(U * gS.unsqueeze(-2), Vh);
    }
    ...
    {
        // gA = U gA V^H
        gA = m >= n ? at::matmul(U, at::matmul(gA, Vh)) : at::matmul(at::matmul(U, gA), Vh);
    }
    ...
    const Tensor& dA, const Tensor& L, const Tensor& V, const bool is_hermitian)
{
    ...
    auto       dL =
        is_hermitian && dA.is_complex() ? at::real(dP.diagonal(0, -2, -1)) : dP.diagonal(0, -2, -1);
...
)
{
    ...
    {
        meta_grad =
            at::cat({at::cat({A, grad}, -1), at::cat({at::zeros_like(A), std::move(A)}, -1)}, -2);
    }
    else
    {
    ...
    const Tensor& dA, const Tensor& det, const Tensor& LU, const Tensor& pivots, const bool use_A_T)
{
    ...
    auto LU_   = LU + at::diag_embed(at::where(LU.diagonal(0, -2, -1) == 0., eps, 0.));
    auto AinvE = at::linalg_lu_solve(LU_, pivots, dA, /*left=*/true, /*adjoint=*/use_A_T);
    ...
    const bool    use_A_T)
{
    ...
    auto trAinvE =
        at::linalg_lu_solve(LU, pivots, dA, /*left*/ true, use_A_T).diagonal(0, -2, -1).sum(-1);
    if (LU.is_complex())
    {
    ...
    {
        return std::make_tuple(at::_efficientzerotensor(sign.sizes(), sign.options()), trAinvE);
    }
}
    ...
    const bool    unitriangular)
{
    return generic_solve_jvp(
        ...
        {
            return std::get<0>(
                at::triangular_solve(dB - dA_contrib, A, upper, transpose, unitriangular));
        },
        X,
        A,
        dA,
    ...
    const c10::SymInt& last_dim_size)
{
    ...
    const auto last_dim = at::maybe_wrap_dim(dim.back(), static_cast<int64_t>(half_sizes.size()));
...
Tensor sinc_backward(const Tensor& grad, const Tensor& self)
{
    ...
    return at::where(self_squared_pi == 0.0, at::scalar_tensor(0.0, grad.options()), out);
}
...
Tensor embedding_dense_double_backward_symint(
    const Tensor& grad, const Tensor& indices, const c10::SymInt& padding_idx)
{
    ...
    const bool    flip_order)
{
    ...
    if (areAnyTensorSubclassLike({input, tau, K}))
    {
        ...
        {
            auto zero_grad_shape = at::SymDimVector(input_.sym_sizes().slice(0, input_.dim() - 1));
...
Tensor i1_backward(const Tensor& grad, const Tensor& self, const Tensor& result)
{
    return AT_DISPATCH_FLOATING_TYPES(
        ...
        [&]()
        {
            ...
            auto safe_self =
                at::where(self_is_not_tiny, self, at::scalar_tensor(eps, self.options()));
    ...
    return AT_DISPATCH_FLOATING_TYPES(
        ...
        [&]()
        {
            ...
            auto safe_self =
                at::where(self_is_not_tiny, self, at::scalar_tensor(eps, self.options()));
            auto gradx =
                (at::special_i0e(safe_self) - result * (safe_self.sgn() + safe_self.reciprocal()));
            return grad *
                   at::where(self_is_not_tiny, gradx, at::scalar_tensor(0.5, self.options()));
        });
}
    ...
    const bool    adjoint)
{
    ...
    at::NoTF32Guard disable_tf32;
    auto [P, L, U] =
        at::lu_unpack(LU, pivots, /*unpack_data=*/true, /*unpack_pivots=*/left == adjoint);
    ...
    else
    {
        ...
        // gU = (L^H gR U^{-H}).triu()
        auto gU =
            at::linalg_solve_triangular(U.mH(), L.mH().matmul(gR), /*upper*/ false, /*left*/ false)
                .triu();
        return gR.tril(-1) + gU;
    }
}
    ...
    const bool    adjoint)
{
    ...
    else
    {
        ...
        auto R =
            at::linalg_solve_triangular(U, L.matmul(dLU.triu()), /*upper*/ true, /*left*/ false) +
            dLU.tril(-1);
...
{
    auto [P, L, U] = at::lu_unpack(LU, pivs, /*unpack_data=*/true, /*unpack_pivots*/ pivot);
    ...
    const Tensor& dA, const Tensor& P, const Tensor& L, const Tensor& U, const bool pivot)
{
    ...
    else if (m < n)
    {
        ...
        auto       dU2  = at::linalg_solve_triangular(
    ...
    else
    {
        ...
        auto       dL2  = at::linalg_solve_triangular(U1, PdA2, /*upper=*/true, /*left=*/false) -
                   L2.matmul(dK.triu());
        return std::make_tuple(
...
Tensor safe_logsumexp_jvp(const Tensor& self_p, const Tensor& self_t, IntArrayRef dim, bool keepdim)
{
    ...
    const auto zero =
        at::scalar_tensor(0.0, at::TensorOptions().dtype(lse_jvp.dtype()).device(lse_jvp.device()));
    ...
    const at::Tensor&            workspace)
{
    ...
    {
        bias_ih = at::zeros({4 /* num_bias_gates of LSTM */ * hidden_size}, weight0.options());
    ...
    for (int64_t seq = 1; seq < seq_length + 1; seq++)
    {
        ...
        auto gate =
            at::linear(input_[x_index], weight0, bias_ih).add_(at::linear(hx, weight1, bias_hh));
