...
{
    CommonSubexpressionEliminator(std::shared_ptr<Graph> graph) : graph_(std::move(graph)) {}
    ...
    bool run(Block* block, std::function<Node*(Node*)> parent_lookup_fn)
    {
        ...
        for (auto it = block->nodes().begin(); it != block->nodes().end(); ++it)
        {
            ...
            {
                GRAPH_DEBUG(
                    "Profiled nodes shouldn't be CSE'ed there's a separate pass that does dedup "
                    "and merging:\n",
                    *node);
                continue;
            }
