...
struct TORCH_API MutationRemover
{
    MutationRemover(
        ...
        : mutation_filter_(std::move(mutation_filter)), aliasDb_(nullptr), graph_(std::move(graph))
    {
    }
    ...
    bool isSpecialMappedOp(Node* n)
    {
        return n->matches("aten::zero_(Tensor(a!) self) -> Tensor(a!)") ||
               n->matches("aten::fill_.Scalar(Tensor(a!) self, Scalar value) -> Tensor(a!)") ||
               n->matches(
...
private:
    ...
    bool  tryMakeCreationAndMutationAtomic(Value* mutated_value, Node* mutating_op);
    bool  tryMakeUnaliasedIfOutputAndMutationAtomic(Value* mutated_value, Node* mutating_op);
    // return true if graph is modified
...
TORCH_API bool InplaceToFunctionalActivation(const std::shared_ptr<Graph>& graph);
