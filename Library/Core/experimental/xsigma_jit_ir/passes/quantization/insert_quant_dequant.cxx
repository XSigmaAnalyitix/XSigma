...
{
    return qscheme == c10::kPerChannelAffine || qscheme == c10::kPerChannelSymmetric;
}
    ...
    const std::string&        quantize_func)
{
    ...
    Node* quant = insertQuant(
        g, inputs, at::Symbol::aten(quantize_func), original_val->debugName() + ".quant");
...
Node* insertEmbeddingBagOps(Node* observer, const std::string& op_name)
{
    ...
    if (is_aten_op)
    {
        TORCH_CHECK(inputs_size == 9, "Expecting FP aten::embedding_bag operator to have 9 inputs");
    ...
    {
        TORCH_CHECK(inputs_size == 12, "Expecting F.embedding_bag operator to have 12 inputs");
    ...
    if (is_aten_op)
    {
        ...
        for (const auto i : c10::irange(1, embedding_bag_float_op->outputs().size()))
        {
            TORCH_CHECK(
    ...
    QuantType quant_type = QuantType::STATIC)
{
    ...
    if (quant_type == QuantType::DYNAMIC)
    {
        ...
        else if (!isWeight(module, observer_out))
        {
            ...
            if (observer_dtype == at::ScalarType::QUInt8 || observer_dtype == at::ScalarType::QInt8)
            {
...
private:
    ...
    void propagateQParams(
        ...
        const std::optional<std::tuple<c10::QScheme, QParamVector>>& qparams_opt = std::nullopt);
...
std::tuple<c10::QScheme, QParamVector> InsertQuantDeQuantHelper::getQSchemeAndQParamVector(
    script::Module& module, Node* n)
{
    ...
    const std::optional<std::tuple<c10::QScheme, QParamVector>>& qparams_opt)
{
    ...
    else
    {
        ...
        Node* scale = insertQParam(
            graph, quantized_input, at::Symbol::aten("q_scale"), FloatType::get(), "q_scale");
...
void checkCalculateQParamsResultTypes(const Node* out)
{
    ...
    TORCH_CHECK(scale->type()->expect<TensorType>(), "Scale value should be of Tensor type.");
    TORCH_CHECK(zp->type()->expect<TensorType>(), "Scale value should be of float type.");
}
