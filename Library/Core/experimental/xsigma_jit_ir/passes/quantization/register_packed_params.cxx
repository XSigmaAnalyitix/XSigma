...
std::pair<Value*, std::string> findFPWeight(Node* prepack_node)
{
    ...
    TORCH_CHECK(is_quantize_node, "Input to prepack node must be output of weight quantization.");
    ...
    while (!blocks_to_visit.empty())
    {
        ...
        for (Node* n : b->nodes())
        {
            if (is_packed_param(n))
            {
                ...
                auto        empty_tensor = at::empty({0}, at::TensorOptions().requires_grad(false));
