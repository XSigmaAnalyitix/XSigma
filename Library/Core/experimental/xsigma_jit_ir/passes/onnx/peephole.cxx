...
static std::optional<size_t> fusibleExpandTo(at::IntArrayRef from, at::IntArrayRef to)
{
...
static void pushPackingPastRnn(Block* b)
{
    for (auto it = b->nodes().begin(); it != b->nodes().end(); ++it)
    {
        ...
        while (!batch_sizes->uses().empty())
        {
            ...
            {
                const at::Tensor& const_val_t = user->inputs().at(1)->node()->t(attr::value);
