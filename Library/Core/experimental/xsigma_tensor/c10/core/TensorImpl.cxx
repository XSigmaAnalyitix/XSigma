...
C10_DEFINE_bool(
    caffe2_keep_on_shrink, true, "If set, keeps memory when a tensor is shrinking its size.")
        ...
        namespace c10
{
    const char* const TensorImpl::err_msg_tensor_metadata_change_not_allowed =
        ...
        "If your intent is to change the metadata of a Tensor (such as sizes / strides / storage / "
        "storage_offset)\n"
        "without autograd tracking the change, remove the .data / .detach() call and wrap the "
        "change in a `with torch.no_grad():` block.\n"
        "For example, change:\n"
        "    x.data.set_(y)\n"
        "to:\n"
        "    with torch.no_grad():\n"
    ...
    const at::Tensor& TensorImpl::_fw_grad(uint64_t level, const at::TensorBase& self) const
    {
    ...
    TensorImpl::TensorImpl(
        Storage && storage, DispatchKeySet key_set, const caffe2::TypeMeta data_type)
    ...
    TensorImpl::TensorImpl(
        ...
        const caffe2::TypeMeta     data_type,
        std::optional<c10::Device> device_opt)
        : storage_(std::move(storage)), numel_(0), data_type_(data_type), device_opt_(device_opt)
    {
        init_bitfields();
    ...
    void TensorImpl::HandleResize()
    {
        ...
        if (reserved_)
        {
            ...
            reset_tensor = storage_.nbytes() < (storage_offset_ + numel_) * data_type_.itemsize();
        }
        else
        {
            reset_tensor = storage_.nbytes() < (storage_offset_ + numel_) * data_type_.itemsize() ||
    ...
    bool TensorImpl::compute_contiguous() const
    {
        ...
        return _compute_contiguous<int64_t>(
            sizes_and_strides_.sizes_arrayref(), sizes_and_strides_.strides_arrayref(), numel_);
    }
    ...
    bool TensorImpl::compute_channels_last_contiguous_2d() const
    {
        ...
        return _compute_channels_last_contiguous_2d<int64_t>(
            sizes_and_strides_.sizes_arrayref(), sizes_and_strides_.strides_arrayref());
    }
    ...
    void TensorImpl::throw_data_ptr_access_error() const
    {
        ...
        TORCH_CHECK(false, "Cannot access data pointer of Tensor that doesn't have storage");
    }

    c10::SymBool TensorImpl::sym_is_contiguous_custom(at::MemoryFormat memory_format) const
    {
    ...
    c10::Layout TensorImpl::layout_custom() const
    {
        ...
        // TODO: fix this
        TORCH_CHECK(0, "Tensors of type ", tensorimpl_type_name(), " do not have layout")
    ...
    int64_t TensorImpl::storage_offset_custom() const
    {
        ...
        {
            // TODO: fix this
            return pyobj_slot_.load_pyobj_interpreter()->sym_storage_offset(this).guard_int(
                __FILE__, __LINE__);
        }
    ...
    at::DataPtr PlacementDeleteContext::makeDataPtr(
        at::DataPtr && data_ptr, PlacementDtor placement_dtor, size_t size, at::Device device)
    {
    ...
    // tensor.
    void TensorImpl::copy_generic_tensor_metadata(const TensorImpl* src_impl, TensorImpl* dest_impl)
    {
        ...
        const TensorImpl* src_impl, TensorImpl* dest_impl, bool allow_tensor_metadata_change)
    {
        ...
        dest_impl->key_set_ =
            (src_impl->key_set_ - c10::python_ks) | (dest_impl->key_set_ & c10::python_ks);
    ...
    void TensorImpl::Extend(int64_t num, float growthPct)
    {
        ...
        TORCH_CHECK(is_contiguous_, "Right now Extend is only supported for contiguous Tensor.");
        TORCH_CHECK(!has_symbolic_sizes_strides_, "Extend() called on tensor with symbolic shape")
        ...
        if (data_type_.copy())
        {
            TORCH_CHECK(device_type() == DeviceType::CPU, "non-POD types work only on CPU");
    ...
    {
        TORCH_CHECK(
            is_contiguous_, "Right now ReserveSpace is only supported for contiguous Tensor.");
        TORCH_CHECK(
            !has_symbolic_sizes_strides_, "ReserveSpace() called on tensor with symbolic shape")

        TORCH_CHECK(storage_.unique(), "Can't call ReserveSpace on shared storage.");
        // TODO: eliminate newCapacity.
        IntArrayRef             sizes_and_strides = sizes_and_strides_.sizes_arrayref();
        SmallVector<int64_t, 5> newCapacity(sizes_and_strides.begin(), sizes_and_strides.end());
        ...
        auto                    oldSize = numel_;
        SmallVector<int64_t, 5> oldDims(sizes_and_strides.begin(), sizes_and_strides.end());
        Resize(std::move(newCapacity));
    ...
    {
        TORCH_CHECK(is_contiguous_, "Right now Reshape is only supported for contiguous Tensor.");
        TORCH_CHECK(!has_symbolic_sizes_strides_, "Reshape() called on tensor with symbolic shape")
    ...
    void TensorImpl::ShareData(const TensorImpl& src)
    {
        ...
        TORCH_CHECK(
            src.numel_ == numel_, "Size mismatch - did you call reshape before sharing the data?");
        ...
        {
            C10_LOG_EVERY_MS(WARNING, 1000) << "Source tensor don't have a data type (did you call "
                                               "mutable_data<T> on the tensor?)";
        }
        TORCH_CHECK(src.storage_initialized(), "Source tensor has no content and has size > 0");
        // Finally, do sharing.
   ...
   */
        ...
        std::optional<c10::SymInt> storage_offset)
    {
        ...
        if (!extra_meta_)
        {
            ...
            extra_meta_->symbolic_shape_meta_ = std::make_unique<c10::SymbolicShapeMeta>();
    ...
    void TensorImpl::generic_set_sizes_contiguous(SymIntArrayRef sizes)
    {
        ...
        refresh_numel();
        empty_tensor_restride_symint(MemoryFormat::Contiguous);  // calls refresh_contiguous()
    }
    ...
    void TensorImpl::empty_tensor_restride_symint(MemoryFormat memory_format)
    {
        ...
        case MemoryFormat::Contiguous:
        {
            ...
            if (dim_ > 0)
            {
                ...
                {
                    sym_shape_meta.strides_[i] =
                        sym_shape_meta.strides_[i + 1] * sym_shape_meta.sizes_[i + 1].max(1);
                }
            }
            break;
        }
        case MemoryFormat::ChannelsLast:
        {
            ...
            clone_symvec(get_channels_last_strides_2d(sym_sizes()), sym_shape_meta.strides_);
            break;
        }
        case MemoryFormat::ChannelsLast3d:
        {
            TORCH_CHECK(dim() == 5, "required rank 5 tensor to use channels_last_3d format");
            clone_symvec(get_channels_last_strides_3d(sym_sizes()), sym_shape_meta.strides_);
            break;
        }
