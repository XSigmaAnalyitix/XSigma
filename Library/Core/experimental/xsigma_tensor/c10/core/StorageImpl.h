...
public:
    ...
    StorageImpl(
        ...
        : StorageImpl(
              use_byte_size_t(),
              size_bytes,
              size_bytes.is_heap_allocated() ? allocator->allocate(0)
    ...
    void release_resources() override { data_ptr_.clear(); }
    ...
    SymInt sym_nbytes() const { return size_bytes_; }
    ...
    void set_nbytes(c10::SymInt size_bytes) { size_bytes_ = std::move(size_bytes); }
    ...
    at::DataPtr& _mutable_data_ptr_no_checks() { return data_ptr_; }
    ...
    at::DeviceType device_type() const { return data_ptr_.device().type(); }

    at::Allocator* allocator() { return allocator_; }

    const at::Allocator* allocator() const { return allocator_; }
    ...
    // set)
    void set_allocator(at::Allocator* allocator) { allocator_ = allocator; }
   ...
   */
    void UniqueStorageShareExternalPointer(void* src, size_t size_bytes, DeleterFnPtr d = nullptr)
    {
        UniqueStorageShareExternalPointer(at::DataPtr(src, src, d, data_ptr_.device()), size_bytes);
    }

    /**
   * Can only be called when use_count is 1
   */
    void UniqueStorageShareExternalPointer(at::DataPtr&& data_ptr, size_t size_bytes)
    {
        data_ptr_                     = std::move(data_ptr);
        size_bytes_                   = static_cast<int64_t>(size_bytes);
        size_bytes_is_heap_allocated_ = false;
    ...
    void set_received_cuda(bool received_cuda) { received_cuda_ = received_cuda; }

    bool received_cuda() { return received_cuda_; }
    ...
    void release_data_and_set_meta_custom_data_ptr_error_msg_(std::optional<std::string> s)
    {
...
private:
    ...
    inline bool is_cow() const { return c10::impl::cow::is_cow_data_ptr(data_ptr_); }
