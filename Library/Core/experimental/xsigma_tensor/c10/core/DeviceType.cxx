...
std::string get_privateuse1_backend(bool lower_case)
{
    ...
    auto backend_name = name_registered ? privateuse1_backend_name : "privateuseone";
    auto op_case      = lower_case ? ::tolower : ::toupper;
    std::transform(backend_name.begin(), backend_name.end(), backend_name.begin(), op_case);
    return backend_name;
}
...
void register_privateuse1_backend(const std::string& backend_name)
{
    ...
    TORCH_CHECK(
        !privateuse1_backend_name_set.load() || privateuse1_backend_name == backend_name,
        "torch.register_privateuse1_backend() has already been set! Current backend: ",
        privateuse1_backend_name);

    static const std::array<std::string, 6> types = {"cpu", "cuda", "hip", "mps", "xpu", "mtia"};
    TORCH_CHECK(
