...
function(configure_sanitizer_flags)
    ...
    list(APPEND SANITIZER_COMPILE_FLAGS "-O3")
...
function(configure_sanitizer_suppressions)
    ...
    if(XSIGMA_SANITIZER_TYPE STREQUAL "address")
        ...
        if(EXISTS ${SUPPRESSION_FILE})
            set(ENV{ASAN_OPTIONS} "suppressions=${SUPPRESSION_FILE}:halt_on_error=1:abort_on_error=1:print_stats=1:print_stacktrace=1")
    ...
    elseif(XSIGMA_SANITIZER_TYPE STREQUAL "undefined")
        ...
        if(EXISTS ${SUPPRESSION_FILE})
            set(ENV{UBSAN_OPTIONS} "suppressions=${SUPPRESSION_FILE}:halt_on_error=1:abort_on_error=1:print_stats=1:print_stacktrace=1")
    ...
    elseif(XSIGMA_SANITIZER_TYPE STREQUAL "thread")
        ...
        if(EXISTS ${SUPPRESSION_FILE})
            set(ENV{TSAN_OPTIONS} "suppressions=${SUPPRESSION_FILE}:halt_on_error=1:abort_on_error=1:print_stats=1:print_stacktrace=1")
            message(STATUS "ThreadSanitizer suppressions: ${SUPPRESSION_FILE}")
        endif()
    elseif(XSIGMA_SANITIZER_TYPE STREQUAL "memory")
        set(SUPPRESSION_FILE "${SCRIPTS_DIR}/msan_suppressions.txt")
        if(EXISTS ${SUPPRESSION_FILE})
            set(ENV{MSAN_OPTIONS} "suppressions=${SUPPRESSION_FILE}:halt_on_error=1:abort_on_error=1:print_stats=1:print_stacktrace=1")
    ...
    elseif(XSIGMA_SANITIZER_TYPE STREQUAL "leak")
        ...
        if(EXISTS ${SUPPRESSION_FILE})
            set(ENV{LSAN_OPTIONS} "suppressions=${SUPPRESSION_FILE}:halt_on_error=1:abort_on_error=1:print_stats=1:print_stacktrace=1")
