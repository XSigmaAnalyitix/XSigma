...
class XSigmaConfigureDetector:
    ...
    def analyze_directory(self, directory: Path, recursive: bool = True) -> List[Dict]:
        ...
        for file_path in directory.glob(pattern):
            if file_path.is_file() and file_path.suffix in self.CPP_EXTENSIONS:
                ...
                if any(part in str(file_path).lower() for part in ['thirdparty', 'third_party', '3rdparty']):
                    continue
        ...
        if self.files_needing_configure > 0:
            ...
            for result in results:
                if result['needs_configure']:
                    ...
                    report_lines.append(f"  Macros found: {', '.join(sorted(result['macros_found']))}")
                    if result['suggestion']:
                        report_lines.append(f"  Suggested insertion at line {result['suggestion']['line']}: {result['suggestion']['include']}")
                    report_lines.append("")
...
def main():
    ...
    parser.add_argument(
        '--log-file',
        type=str,
        help='Log file path for detailed output'
    )
    parser.add_argument(
        '--recursive',
        action='store_true',
    ...
    # Analyze directory
    detector.logger.info(f"Starting analysis of {args.directory}")
    ...
    if args.report_file:
        with open(args.report_file, 'w') as f:
            f.write(report)
        detector.logger.info(f"Report written to {args.report_file}")
    else:
        print(report)
    
    # Exit with appropriate code
    if detector.files_needing_configure > 0:
        detector.logger.warning(f"{detector.files_needing_configure} files need common/configure.h")
        sys.exit(1)
    else:
