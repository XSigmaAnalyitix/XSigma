    ...
    bool                           log_softmax)
{
    ...
    size_t  softmax_dim = normalizeAndCheckIndex(std::get<int64_t>(inputs[1]), rank);
    ...
    auto max = Reduce(
        ...
        { return tensorOrConstant(inputs[0], move_softmax_dim_index_to_pos(indices)); },
    ...
    auto e = Compute(
        ...
        [&](ParameterList& indices)
        {
            auto inp = tensorOrConstant(inputs[0], convert_indices_to_expr_handle(indices));
    ...
    auto sum = Reduce(
        ...
        std::nullopt,
        Sum(),
        [&](ParameterList& indices) { return e.load(move_softmax_dim_index_to_pos(indices)); },
        {outputShape[softmax_dim]});
    if (!log_softmax)
    {
        auto result = Compute(
        ...
        return Tensor(
            result.buf(),
            alloc<tensorexpr::Block>(
                std::vector<StmtPtr>({max.stmt(), e.stmt(), sum.stmt(), result.stmt()})));
    }
    ...
    auto result = Compute(
        ...
        [&](ParameterList& indices)
        {
            ...
            return inp - max.load(non_softmax_indices) - log_sum.load(non_softmax_indices);
        });
    return Tensor(
        result.buf(),
