...
{
public:
    HasRand(StmtPtr stmt) : stmt_(std::move(stmt)) { stmt_->accept(this); }

    bool has_rand() const { return has_rand_; }
...
public:
    ...
    const std::unordered_set<VarPtr>& vars() { return vars_; }
...
public:
    ...
    const std::unordered_set<BufPtr>& bufs() { return bufs_; }
...
public:
    ...
    std::vector<StmtPtr> writes() { return writes_; }
...
public:
    ...
    std::vector<StmtPtr> reads() { return reads_; }
...
private:
    std::tuple<int32_t, int32_t> getLiveRange() { return std::make_tuple(begin_, end_); }
