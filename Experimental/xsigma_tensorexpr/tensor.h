...
{
public:
    Tensor(BufPtr buf, const std::vector<VarPtr>& args, const ExprPtr& body) : buf_(std::move(buf))
    {
    ...
    Tensor(BufPtr buf, StmtPtr stmt) : buf_(std::move(buf)), stmt_(std::move(stmt)) {}
...
inline std::vector<VarHandle> create_index_vars(const std::vector<ExprHandle>& dims)
{
    ...
    {
        vars.emplace_back(
            alloc<Var>("i", dim.dtype().scalar_type() == ScalarType::Long ? kLong : kInt));
    }
    return vars;
}
    ...
    const std::vector<ExprHandle>&                reduce_dims)
{
    ...
    if (reduce_vars.empty())
    {
        ...
        BufHandle  func_result = Buf::make(func_name, dims, body.dtype(), std::nullopt, strides);
    ...
    if (body.dtype() == kBFloat16)
    {
        ...
        BufHandle  func_result_acc = Buf::make(func_name + "_acc", dims, kFloat, init_expr_acc);
        reduce_op =
    ...
    const std::vector<ExprHandle>& reduce_dims)
{
    return Reduce<InitFunc, BodyFunc>(
        func_name, dims, std::nullopt, reducer, init_func, body_func, reduce_dims);
}
...
Tensor Reduce(
    ...
    const std::vector<ExprHandle>& reduce_dims)
{
    return Reduce<BodyFunc>(func_name, dims, std::nullopt, reducer, body_func, reduce_dims);
}
