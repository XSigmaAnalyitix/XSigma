...
class XSIGMA_VISIBILITY CUDAMethods : public ProfilerStubs
{
    void record(
        xsigma::device_enum* device, ProfilerVoidEventStub* event, int64_t* cpu_ns) const override
    {
        if (device)
        {
        ...
        *event = std::shared_ptr<CUevent_st>(
            cuda_event_ptr, [](CUevent_st* ptr) { XSIGMA_CUDA_CHECK(cudaEventDestroy(ptr)); });
    ...
    void synchronize() const override { XSIGMA_CUDA_CHECK(cudaDeviceSynchronize()); }

    bool enabled() const override { return true; }
};

class XSIGMA_VISIBILITY RegisterCUDAMethods
{
    RegisterCUDAMethods()
    {
