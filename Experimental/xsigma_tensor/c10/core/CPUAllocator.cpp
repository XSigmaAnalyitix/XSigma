    ...
    namespace c10
{
    ...
    public:
        ...
        bool is_simple_data_ptr(const c10::DataPtr& data_ptr) const final
        {
            return reinterpret_cast<const uint8_t*>(data_ptr.get()) ==
                   reinterpret_cast<const uint8_t*>(data_ptr.get_context()) + PreGuardBytes;
        }
    ...
    void ProfiledCPUMemoryReporter::New(void* ptr, size_t nbytes)
    {
        ...
        {
            LOG(INFO) << "C10 alloc " << nbytes << " bytes, total alloc " << allocated << " bytes.";
        }
        if (profile_memory)
        {
            reportMemoryUsageToProfiler(
                ptr, static_cast<int64_t>(nbytes), allocated, 0, c10::Device(c10::DeviceType::CPU));
        }
    }
    ...
    void ProfiledCPUMemoryReporter::OutOfMemory(size_t nbytes)
    {
        ...
        if (profile_memory)
        {
            reportOutOfMemoryToProfiler(
                static_cast<int64_t>(nbytes), allocated, 0, c10::Device(c10::DeviceType::CPU));
        }
    }
    ...
    Allocator* GetCPUCachingAllocator()
    {
        ...
        {
            VLOG(1) << "There is not caching allocator registered for CPU, use the default "
                       "allocator instead.";
            return GetAllocator(DeviceType::CPU);
        }
