...
#define DEFINE_IMPLICIT_CTOR(type, name) \
    ...
    static_assert(std::is_same_v<long long, int64_t>, "int64_t is the same as long long on MacOS");
    Scalar(long vv) : Scalar(vv, true) {}
#endif
#if defined(_MSC_VER)
    static_assert(
        std::is_same_v<long long, int64_t>, "int64_t is the same as long long on Windows");
    Scalar(long vv) : Scalar(vv, true) {}
#endif
...
#undef DEFINE_IMPLICIT_CTOR
    ...
    // problem.
    template <typename T, typename std::enable_if_t<std::is_same_v<T, bool>, bool>* = nullptr>
    ...
    template <
        typename T,
        typename std::enable_if_t<std::is_same_v<T, c10::SymBool>, bool>* = nullptr>
...
#undef DEFINE_ACCESSOR
    ...
    SymInt toSymInt() const
    {
        ...
        {
            return c10::SymInt(
                intrusive_ptr<SymNodeImpl>::reclaim_copy(static_cast<SymNodeImpl*>(v.p)));
        }
        else
        {
            return toLong();
    ...
    bool isFloatingPoint() const { return Tag::HAS_d == tag || Tag::HAS_sd == tag; }

    [[deprecated(
        "isIntegral is deprecated. Please use the overload with 'includeBool' parameter "
        "instead.")]] bool
    isIntegral() const
    {
    ...
    bool isUnsigned() const { return Tag::HAS_u == tag || (Tag::HAS_i == tag && v.i >= 0); }

    bool isComplex() const { return Tag::HAS_z == tag; }
    bool isBoolean() const { return Tag::HAS_b == tag || Tag::HAS_sb == tag; }

    // you probably don't actually want these; they're mostly for testing
    bool isSymInt() const { return Tag::HAS_si == tag; }
    bool isSymFloat() const { return Tag::HAS_sd == tag; }
    bool isSymBool() const { return Tag::HAS_sb == tag; }
    ...
    template <typename T, typename std::enable_if_t<!c10::is_complex<T>::value, int> = 0>
...
private:
    ...
    C10_ALWAYS_INLINE void moveFrom(Scalar&& rhs) noexcept
    {
        v   = rhs.v;
        tag = rhs.tag;
        if (rhs.tag == Tag::HAS_si || rhs.tag == Tag::HAS_sd || rhs.tag == Tag::HAS_sb)
        {
            // Move out of scalar
            rhs.tag = Tag::HAS_i;
            rhs.v.i = 0;
    ...
    template <
        typename T,
        typename std::enable_if_t<!std::is_integral_v<T> && !c10::is_complex<T>::value, bool>* =
            nullptr>
    ...
    template <typename T, typename std::enable_if_t<c10::is_complex<T>::value, bool>* = nullptr>
