...
bool runtimeDispatchKeySetHas(DispatchKey t, DispatchKey k)
{
    ...
    case DispatchKey::CompositeExplicitAutogradNonFunctional:
        // See Note [NestedTensor Not Included in Backend Keys]
        return k != DispatchKey::NestedTensor && non_functional_backend_dispatch_keyset.has(k);
...
DispatchKeySet::iterator& DispatchKeySet::iterator::operator++()
{
    ...
    uint64_t masked_backend_bits =
        llvm::maskTrailingZeros<uint64_t>(next_backend_) & full_backend_mask & *data_ptr_;
    ...
    if (isPerBackendFunctionalityKey(static_cast<DispatchKey>(next_dispatchkey_idx)))
    {
        ...
        uint64_t next_backendcomponent_bits =
            llvm::maskTrailingZeros<uint64_t>(first_backendcomponent_idx + 1) & full_backend_mask &
            *data_ptr_;
...
initializeFunctionalityOffsetsAndMasks()
{
    ...
    for (const auto functionality_idx : c10::irange(1, num_functionality_keys))
    {
        ...
        auto next_offset =
            prev_offset_and_mask.offset + (prev_offset_and_mask.mask == 0 ? 1 : num_backends);
    ...
    TORCH_INTERNAL_ASSERT(
        offsets_and_masks[num_functionality_keys - 1].offset == (num_runtime_entries - 1),
        "num_runtime_entries: ",
        num_runtime_entries,
        "last_offset: ",
