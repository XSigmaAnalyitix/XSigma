...
public:
    /* GETTERS */
    bool logGraphBetweenPasses() const { return log_graph_between_passes_; }
...
{
public:
    explicit GraphPassManager(GraphPassPipeline pipeline, PassManagerOptions opts = {});
    ~GraphPassManager() = default;

    bool run(Graph* graph);

    const GraphPassPipeline& pipeline() const { return pipeline_; }

    const PassManagerOptions& opts() const { return opts_; }
