...
static void UpdateDifferentiableGraphRequiresGrad(
    Block* block, std::optional<bool> new_requires_grad)
{
...
void UpdateDifferentiableGraphRequiresGrad(
    std::shared_ptr<Graph>& diff_forward_graph, std::optional<bool> new_requires_grad)
{
    UpdateDifferentiableGraphRequiresGrad(diff_forward_graph->block(), new_requires_grad);
}
