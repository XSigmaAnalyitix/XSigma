...
bool canRunWithAutograd(Node* node)
{
    ...
    {
        if (!std::all_of(block->nodes().begin(), block->nodes().end(), canRunWithAutograd))
        {
            return false;
        }
    }
    return kind != prim::FusionGroup && kind != prim::CudaFusionGroup && kind != prim::TypeCheck &&
           kind != prim::TensorExprGroup && kind != prim::CudaFusionGuard &&
           kind != prim::oneDNNFusionGroup && kind != prim::oneDNNFusionGuard &&
...
graph_node_list::iterator scanNode(Node* node, size_t threshold)
{
    ...
    if (!std::all_of(subgraph->nodes().begin(), subgraph->nodes().end(), canRunWithAutograd))
    {
        return next_node;
    }
