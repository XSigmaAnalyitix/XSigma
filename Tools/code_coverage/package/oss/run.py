...
def clang_run(
    tests: TestList, build_folder: str, test_subfolder: str = "bin"
) -> None:
    start_time = time.time()
    for test in tests:
        # raw_file
        raw_file = os.path.join(
            get_raw_profiles_folder(), test.name + ".profraw"
        )
        # binary file
        binary_file = get_oss_binary_file(
            test.name, test.test_type, build_folder, test_subfolder
        )
...
def gcc_run(
    ...
    for test in tests:
        ...
        gcc_coverage.run_target(
            binary_file, test.test_type, build_folder, test_subfolder
        )
