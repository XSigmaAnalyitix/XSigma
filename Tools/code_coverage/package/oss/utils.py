...
def get_oss_shared_library(
    ...
    Searches for platform-specific shared libraries (.dll on Windows,
    .so on Linux, .dylib on macOS) in both lib/ and bin/ directories.
    ...
    Returns:
        List of shared library paths (empty list if none found or build folder
        doesn't exist)
    """
    import platform
    ...
    # Input validation
    if not build_folder:
        print_error("build_folder cannot be empty")
        return []
    ...
    # Validate that build path exists
    if not os.path.isdir(build_path):
    ...
    # Log if no library directories found
    if not search_dirs:
        print_error(f"No library directories found in {build_path}")
        return []
    ...
    for search_dir in search_dirs:
        try:
            ...
            print_error(f"Failed to list libraries in {search_dir}: {e}")
...
def get_llvm_tool_path() -> str:
    ...
    search_paths = {
        "Windows": [
            ...
            "C:\\tools\\llvm\\bin",
            "C:\\msys64\\clang64\\bin",  # MSYS2 installation
            "C:\\scoop\\apps\\llvm\\current\\bin",  # Scoop package manager
            "C:\\vcpkg\\installed\\x64-windows\\bin",  # vcpkg installation
        ],
        "Darwin": [  # macOS
...
def clean_up_gcda() -> None:
    """Remove all .gcda coverage files from the build folder.

    This is typically called before running GCC coverage tests to ensure
    clean coverage data collection.
    """
...
def get_gcda_files() -> list[str]:
    """Find all .gcda coverage files in the build folder.

    Searches for .gcda files in the build folder, which are generated by
    GCC when running tests with coverage instrumentation. Supports both
    relative and absolute build folder paths.

    Returns:
        List of paths to .gcda files found in the build folder
    """
    import os
    ...
    if os.path.isdir(folder_has_gcda):
        # Use glob for cross-platform compatibility (replaces Unix-only find command)
        import glob
        pattern = os.path.join(folder_has_gcda, "**", "*.gcda")
        output = glob.glob(pattern, recursive=True)
        return output
    else:
        return []
...
def run_oss_python_test(
    ...
    try:
        ...
        print_error(f"Failed to execute Python test {binary_file}: {e}")
        return False
